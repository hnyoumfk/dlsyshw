...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    #return np.average(np.log(np.sum(np.exp(Z), axis=1)) - np.choose(y,np.transpose(Z)))
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0,m,batch):
        ...
        # Get Batch Size
        size = batchI.shape[0]
        # Get Step  (BatchX)T * ( exp(BatchX * theta)/sum(exp(BatchX * theta)) ) - BatchI
        z = np.exp( np.matmul(batchX ,theta))
        batchLoss = np.matmul(batchX.T , ( z / np.sum(z, axis=1).reshape((size,1)) - batchI ) ) / size
        
        # Add Step to Loss Sum
        theta -= lr * batchLoss 
