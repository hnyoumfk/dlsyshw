...
class PowerScalar(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        a = node.inputs[0]
        return self.scalar* array_api.power(a, self.scalar-1)
        ### END YOUR SOLUTION
...
class Transpose(TensorOp):
    ...
    def compute(self, a):
        ### BEGIN YOUR SOLUTION
        axis_list = list(range(len(array_api.shape(a))))
        axis_list[self.axes[1]] , axis_list[self.axes[0]] = axis_list[self.axes[0]] , axis_list[self.axes[1]]
        return array_api.transpose(a, axes=axis_list)
        ### END YOUR SOLUTION
...
class BroadcastTo(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        a = node.inputs[0]
        rank_offset = len(out_grad.shape) - len(a.shape)
        axes = []
        for i in range(rank_offset):
            axes.append(i)
        for i in range(len(a.shape)):
            if a.shape[i] == 1 and out_grad.shape[rank_offset + i]!=1:
                axes.append(rank_offset + i)
        g = reshape(summation(out_grad, tuple(axes)), a.shape) 
...
class MatMul(TensorOp):
    ...
    def gradient(self, out_grad, node):
        ...
        gl = matmul(out_grad, transpose(rhs))
        for _ in range(rrank - lrank):
            gl = summation(gl, (0))

        gr = matmul(transpose(lhs), out_grad )
        for _ in range(lrank - rrank):
            gr = summation(gr, (0))
        return gl , gr
        ### END YOUR SOLUTION
